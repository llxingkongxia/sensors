...
/*Proximity Part*/
/******************/
static struct regulator *reg_vio, *reg_vdd;

static int cm36656_regulator_init(void){
	int ret = 0;

	reg_vio = regulator_get(&g_i2c_client->dev,"vio");
    if (IS_ERR_OR_NULL(reg_vio)) {
        ret = PTR_ERR(reg_vio);
        err("Failed to get regulator vcc_ana %d\n", ret);
        return ret;
    }
    ret = regulator_set_voltage(reg_vio, 1800000, 1950000);
    if (ret) {
        err("Failed to set voltage for vcc_ana reg %d\n", ret);
        return -1;
    }

	reg_vdd = regulator_get(&g_i2c_client->dev,"vdd");
    if (IS_ERR_OR_NULL(reg_vdd)) {
        ret = PTR_ERR(reg_vdd);
        err("Failed to get regulator vcc_ana %d\n", ret);
        return ret;
    }
    ret = regulator_set_voltage(reg_vdd, 3000000, 3600000);
    if (ret) {
        err("Failed to set voltage for vcc_ana reg %d\n", ret);
        return -1;
    }
...
static int cm36656_regulator_enable(void){
    ...
    ret = regulator_set_load(reg_vio, 10000);
    if (ret < 0) {
    ...
    for(idx=0; idx<10; idx++){
        if(regulator_is_enabled(reg_vio) > 0){
    ...
    ret = regulator_set_load(reg_vdd, 10000);
    if (ret < 0) {
        err("Failed to set load for vcc_ana reg %d\n", ret);
        return ret;
    }
...
static int cm36656_regulator_disable(void){
    ...
    ret = regulator_set_load(reg_vdd, 0);
    if (ret < 0) {
        err("Failed to set load for vcc_ana reg %d\n", ret);
        return ret;
    }
    ...
    ret = regulator_set_load(reg_vio, 0);
    if (ret < 0) {
        err("Failed to set load for vcc_ana reg %d\n", ret);
        return ret;
    }
